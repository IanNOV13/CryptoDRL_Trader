...
from tensorflow.keras.layers import Conv1D, MaxPooling1D, LSTM, Dense, LeakyReLU, Input , Concatenate, Add, GlobalAveragePooling1D # type: ignore
from tensorflow.keras.layers import BatchNormalization # type: ignore
import numpy as np
# --- 你的 Inception 模塊實現 ---
def inception_module(input_tensor):
    # 使用 128 個濾波器作為 bottleneck 和主要卷積層的濾波器數量
    # MaxPooling 分支的 bottleneck 使用 128 個濾波器
    num_filters_conv = 128
    num_filters_bottleneck = 128
...
# --- 新的模型構建函數 (使用你提供的模塊) ---
def build_inception_lstm_dqn_model(input_shape, action_size):
    """
    使用你提供的 Inception+Residual+lstm 架構作為特徵提取器，
    ...
    x = Conv1D(filters=128, kernel_size=1, activation='relu', padding='same', name='channel_reduction_conv')(x)

    # --- lstm 層 (來自模型 2) ---
    # return_sequences=True 保持序列輸出給下一層 lstm
    x = LSTM(128, return_sequences=True, dropout=0.1, name="lstm_1")(x)
    # 第二層 lstm - 決定如何獲取最終特徵向量
    # 選擇 1: 使用 GlobalAveragePooling1D (同模型 2)
    x = LSTM(128, return_sequences=True, dropout=0.25, name="lstm_2")(x) # 保持 return_sequences=True
    feature_vector = GlobalAveragePooling1D(name="gap_lstm")(x)

    # 選擇 2: 使用最後一個時間步的輸出
    # x = lstm(512, return_sequences=False, dropout=0.1, name="lstm_2")(x) # 設置 return_sequences=False
    # feature_vector = x
...
if __name__ == "__main__":
    model = build_inception_lstm_dqn_model(input_shape=(100, 28), action_size=3)
    model.summary()
