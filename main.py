...
from utils import notify_discord_webhook, clear_lines, setup_gpu # 示例工具函數
...
def main():
    ...
    #    這部分邏輯也可以封裝到 agent.load_checkpoint() 或 utils.load_checkpoint()
    start_episode = config["START_EPISODE"]
    best_reward = config["BEST_REWARD"]
    best_balance = config["BEST_BALANCE"]
    # ...
    if os.path.exists(config["LOAD_MODEL_PATH"]): # 或者更精細的檢查點文件
        agent.load_model(config["LOAD_MODEL_PATH"]) # agent 內部處理主模型和目標模型加載
        print("從檢查點繼續訓練...")
    else:
        print("開始新的訓練...")
    ...
    for episode in range(start_episode, config["EPISODES"]):
        ...
        while not done:
            ...
            if global_step_count % 7 == 0:
                if agent.can_train(): # agent 內部判斷經驗池大小等
                    agent.train_batch() # 內部處理混合抽樣、DDQN Target、model.fit
            ...
            print(f"回合{episode+1}/{config['EPISODES']}，探索機率：{current_epsilon:.4f}，學習率: {current_lr:.6f}")
            print(f"步數：{t+1}/{config['BEST_STEPS']}")
            ...
            print(f"餘額：{train_env.total_balance:6.2f}/{best_balance:6.2f}，餘額歷史最高值：{np.max(train_env.balance_history):6.2f}，最大回撤：{train_env.max_drawdown:6.2f}")
        ...
        report_text += f"回合{episode+1}/{config['EPISODES']}，探索機率：{current_epsilon:.4f}，學習率: {current_lr:.6f}\n"
        report_text += f"步數：{t+1}/{config['BEST_STEPS']}\n"
        ...
        report_text += f"餘額：{train_env.total_balance:6.2f}/{best_balance:6.2f}，餘額歷史最高值：{np.max(train_env.balance_history):6.2f}，最大回撤：{train_env.max_drawdown:6.2f}\n"
        print(report_text)
